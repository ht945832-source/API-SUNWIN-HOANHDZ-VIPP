...
try {
    if (fs.existsSync(PATTERN_LIBRARY_FILE)) {
        patternLibrary = JSON.parse(fs.readFileSync(PATTERN_LIBRARY_FILE, 'utf8'));
    }
} catch(e) { console.error('Lỗi load patterns:', e.message); }
try {
    if (fs.existsSync(AI_WEIGHTS_FILE)) {
        aiWeights = JSON.parse(fs.readFileSync(AI_WEIGHTS_FILE, 'utf8'));
    }
...
class UltimatePatternAnalyzer {
    ...
    analyzePattern11(results) {
        ...
        if (isPerfect && last6.length >= 4) {
            ...
            return { pattern: '1-1', prediction: prediction, confidence: confidence, description: `Cầu 1-1 chuẩn ${last6.length} phiên`, nextMove: prediction };
        }
        let errorCount = 0;
        for (let i = 1; i < last6.length; i++) {
        ...
        if (errorCount === 1 && last6.length >= 5) {
            const last = results[results.length - 1];
            const prediction = last === 'Tài' ? 'Xỉu' : 'Tài';
            return { pattern: '1-1', prediction: prediction, confidence: 0.68, description: 'Cầu 1-1 biến thể (1 lỗi)', nextMove: prediction };
        }
        return null;
    }
    ...
    analyzePattern22(results) {
        ...
        if (isPattern22 && last8.length >= 6) {
            ...
            return { pattern: '2-2', prediction: prediction, confidence: confidence, description: 'Cầu 2-2 (cặp đôi)', nextMove: prediction };
        }
        return null;
    }
    ...
    analyzePattern33(results) {
        ...
        if (isPattern33 && last12.length >= 9) {
            ...
            return { pattern: '3-3', prediction: prediction, confidence: 0.85, description: 'Cầu 3-3 siêu chuẩn', nextMove: prediction };
        }
        return null;
    }
    ...
    analyzePattern121(results) {
        ...
        if (last5[0] !== last5[1] && last5[1] === last5[2] && last5[2] !== last5[3] && last5[3] === last5[4]) {
            ...
            return { pattern: '1-2-1', prediction: prediction, confidence: 0.75, description: 'Cầu 1-2-1 hoàn hảo', nextMove: prediction };
        }
        if (results.length >= 7) {
            ...
            if (last7[0] !== last7[1] && last7[1] === last7[2] && last7[2] !== last7[3] &&
                ...
                return { pattern: '1-2-1', prediction: prediction, confidence: 0.82, description: 'Cầu 1-2-1-2-1 mở rộng', nextMove: prediction };
            }
        }
        return null;
    }
    ...
    analyzePattern212(results) {
        ...
        if (last5[0] === last5[1] && last5[1] !== last5[2] && last5[2] !== last5[3] && last5[3] === last5[4]) {
            ...
            return { pattern: '2-1-2', prediction: prediction, confidence: 0.78, description: 'Cầu 2-1-2 chuẩn', nextMove: prediction };
        }
        return null;
    }
    ...
    analyzePattern1122(results) {
        ...
        if (last6[0] !== last6[1] && last6[1] !== last6[2] && 
            ...
            return { pattern: '1-1-2-2', prediction: prediction, confidence: 0.72, description: 'Cầu 1-1-2-2', nextMove: prediction };
        }
        return null;
    }
    ...
    analyzePattern2211(results) {
        ...
        if (last6[0] === last6[1] && last6[1] !== last6[2] && 
            last6[2] !== last6[3] && last6[3] === last6[4] && last6[4] !== last6[5]) {
            const prediction = last6[5];
            return { pattern: '2-2-1-1', prediction: prediction, confidence: 0.72, description: 'Cầu 2-2-1-1', nextMove: prediction };
        }
        return null;
    }
    ...
    analyzeStreak(results) {
        ...
        if (streak >= 3) {
            ...
            if (streak >= 7) {
                ...
                return { pattern: 'bệt', prediction: opposite, confidence: confidence, description: description, nextMove: opposite };
            }
            return { pattern: 'bệt', prediction: last, confidence: confidence, description: description, nextMove: last };
        }
        return null;
    }
    ...
    analyzePerfectAlternate(results) {
        ...
        if (alternateRatio >= 0.8) {
            const last = results[results.length - 1];
            const prediction = last === 'Tài' ? 'Xỉu' : 'Tài';
            const confidence = Math.min(0.7 + (alternateRatio - 0.8) * 1.5, 0.88);
            return { pattern: 'xen_kẽ', prediction: prediction, confidence: confidence, description: `Cầu xen kẽ ${(alternateRatio * 100).toFixed(0)}%`, nextMove: prediction };
        }
        return null;
    }
    ...
    analyzeTrend(results) {
        ...
        if (taiCount === 5) {
            return { pattern: 'giảm_dần', prediction: 'Xỉu', confidence: 0.68, description: 'Tài đang giảm dần, dự đoán Xỉu', nextMove: 'Xỉu' };
        }
        if (xiuCount === 5) {
            return { pattern: 'giảm_dần', prediction: 'Tài', confidence: 0.68, description: 'Xỉu đang giảm dần, dự đoán Tài', nextMove: 'Tài' };
        }
        ...
        if (last3Tai > prev3Tai + 1) {
            return { pattern: 'tăng_dần', prediction: 'Tài', confidence: 0.62, description: 'Xu hướng Tài tăng dần', nextMove: 'Tài' };
        }
        if (last3Tai < prev3Tai - 1) {
            return { pattern: 'tăng_dần', prediction: 'Xỉu', confidence: 0.62, description: 'Xu hướng Xỉu tăng dần', nextMove: 'Xỉu' };
        }
        return null;
    }
    ...
    analyzeSymmetry(results) {
        ...
        for (let radius = 2; radius <= 4; radius++) {
            ...
            if (symmetric) {
                ...
                const confidence = Math.min(0.7 + (radius * 0.03), 0.85);
                return { pattern: 'đối_xứng', prediction: prediction, confidence: confidence, description: `Cầu đối xứng bán kính ${radius}`, nextMove: prediction };
            }
        }
        return null;
    }
    ...
    analyzeCycle(results) {
        ...
        for (let cycleLen = 2; cycleLen <= 5; cycleLen++) {
            ...
            if (matchCount >= 2) {
                ...
                const confidence = Math.min(0.6 + (matchCount * 0.05) + (cycleLen * 0.02), 0.85);
                return { pattern: 'chu_kỳ', prediction: prediction, confidence: confidence, description: `Chu kỳ ${cycleLen} phiên lặp ${matchCount} lần`, nextMove: prediction };
            }
        }
        return null;
    }
    ...
    analyzeScorePattern(history) {
        ...
        if (lastScore >= 15 && avg <= 11) {
            return { pattern: 'điểm_cao', prediction: 'Xỉu', confidence: 0.68, description: `Điểm ${lastScore} cao bất thường`, nextMove: 'Xỉu' };
        }
        if (lastScore <= 6 && avg >= 10) {
            return { pattern: 'điểm_thấp', prediction: 'Tài', confidence: 0.68, description: `Điểm ${lastScore} thấp bất thường`, nextMove: 'Tài' };
        }
        let trend = 0;
        ...
        if (trend > 5) {
            return { pattern: 'xu_hướng', prediction: 'Xỉu', confidence: 0.58, description: 'Điểm đang tăng nhanh', nextMove: 'Xỉu' };
        }
        if (trend < -5) {
            return { pattern: 'xu_hướng', prediction: 'Tài', confidence: 0.58, description: 'Điểm đang giảm nhanh', nextMove: 'Tài' };
        }
        return null;
    }
    ...
    analyzeTransitionMatrix(results) {
        ...
        if (results.length >= 2) {
            ...
            if (transition && (transition.T + transition.X) >= 3) {
                ...
                if (taiProb > 0.65) {
                    return { pattern: 'ma_trận', prediction: 'Tài', confidence: 0.6 + taiProb * 0.1, description: `Ma trận: ${key} → Tài (${(taiProb*100).toFixed(0)}%)`, nextMove: 'Tài' };
                }
                if (xiuProb > 0.65) {
                    return { pattern: 'ma_trận', prediction: 'Xỉu', confidence: 0.6 + xiuProb * 0.1, description: `Ma trận: ${key} → Xỉu (${(xiuProb*100).toFixed(0)}%)`, nextMove: 'Xỉu' };
                }
            }
        }
        return null;
    ...
    analyzePatternLibrary(results) {
        ...
        if (patternLibrary[last5] && patternLibrary[last5].confidence > 0.6) {
            return { pattern: 'thư_viện', prediction: patternLibrary[last5].next, confidence: patternLibrary[last5].confidence, description: `Pattern lưu trữ: ${last5}`, nextMove: patternLibrary[last5].next };
        }
        return null;
    }
    ...
    analyzeAll(history) {
        ...
        if (results.length < 5) {
            return { prediction: 'Tài', confidence: 0.5, pattern: 'chưa_đủ_dữ_liệu', description: 'Đang thu thập dữ liệu...', details: [] };
        }
        const analyses = [];
        const methods = [
        ...
        if (analyses.length === 0) {
            ...
            return { prediction: prediction, confidence: 0.52, pattern: 'cơ_bản', description: 'Phân tích cơ bản', details: [] };
        }
    ...
    savePattern(results, prediction, confidence, actual) {
        ...
        patternLibrary[last5].next = patternLibrary[last5].confidence > 0.55 ? prediction : (patternLibrary[last5].next === 'Tài' ? 'Xỉu' : 'Tài');
        const entries = Object.entries(patternLibrary);
        if (entries.length > 200) {
            const sorted = entries.sort((a, b) => b[1].count - a[1].count);
            const newLibrary = {};
            for (let i = 0; i < 150; i++) {
                newLibrary[sorted[i][0]] = sorted[i][1];
            }
        ...
        ws.on('message', (data) => {
            try {
                ...
                if (cmd === 1003 && d1 && d2 && d3) {
                    ...
                    console.log(`\n🎲 Phiên ${currentSessionId} | KQ: ${result} | Tổng: ${total}`);
                    console.log(`🔍 Phát hiện: ${analysis.pattern} | ${analysis.description}`);
                    console.log(`🎯 Dự đoán phiên tới: ${finalPrediction} (${(finalConfidence*100).toFixed(0)}%) | ${correct ? '✅ ĐÚNG' : '❌ SAI'}`);
                    console.log(`📈 Thống kê: ${stats.correct}/${stats.total} (${tiLe})`);
                    if (analysis.details && analysis.details.length > 0) {
                        console.log(`📊 Chi tiết: ${analysis.details.join(' | ')}`);
                    }
                    console.log('');
...
app.get('/api/history', (req, res) => {
    ...
    const recent = resultHistory.slice(-limit).reverse();
    res.json(recent);
});
...
app.get('/', (req, res) => {
    res.json({ 
        name: 'Tài Xỉu Ultimate API', 
        version: '3.0', 
        author: '@tranhoang2286',
        endpoints: ['/api/ditmemaysun', '/api/stats', '/api/patterns', '/api/history', '/health']
    });
});
...
// Graceful shutdown
process.on('SIGTERM', () => {
    console.log('Nhận SIGTERM, đóng server...');
    server.close(() => {
        if (ws) ws.close();
        process.exit(0);
    });
